...
if( !empty($requestData['search']['value']) ) {
    $query.=" AND (ID_PERFIL LIKE '%".$requestData['search']['value']."%' ";
    $query.=" OR DESC_PERFIL LIKE '%".$requestData['search']['value']."%' )";
}
...
while ($retorno=mysqli_fetch_array ( $result )){
    $id		= $retorno['ID_PERFIL'];
    $nome   = utf8_encode($retorno['DESC_PERFIL']);
    $ativo  = $retorno['ATIVO'];
    if ($ativo=='0'){
        $ativo='Inativo';
    }else{
