...
if( !empty($requestData['search']['value']) ) {
    $query.=" AND (ID_GRUP LIKE '%".$requestData['search']['value']."%' ";
    $query.=" OR DESC_GRUP LIKE '%".$requestData['search']['value']."%' )";
}
...
while ($retorno=mysqli_fetch_array ( $result )){
    $id		= $retorno['ID_GRUP'];
    $nome   = utf8_encode($retorno['DESC_GRUP']);
    $ativo  = $retorno['ATIVO'];
    if ($ativo=='0'){
        $ativo='Inativo';
    }else{
